...
for lrc = 1:length(lr)
    ...
    for reversal = 1:n
        ...
        dataRange = ((pointBefore + reversalPoint):tenPercentPoints(reversal));
        thisData = mainData(reversal,dataRange);
        thisOutput = output(dataRange);
        if(tenPercentPoints(reversal) == -1)
            corrPerReversal(reversal) = -1;
        else
            corrPerReversal(reversal) = corr(thisData(~isnan(thisData))',max(thisOutput(~isnan(thisData)),0)');
        end
    end
